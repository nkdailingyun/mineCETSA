...
8. Try to perform dose/time-response function fitting to generate fitting parameters for each entry/curve using `ms_ITDR_fitting()` or `ms_ITTR_fitting()`.  
...
10. Carry out data segregation and positive hits selection based on desired `fc` level and the associated fitting parameters using `ms_ITDR_filter()`.  
The arguments and its default value includes:  
...
* `excludelastpoint`, exclude the last dose point for dose-response checking, default set to FALSE, only set to TRUE when you believe that that last dose point is not trustworthy (not applied on ms_ITTR_ggplotting function)  
...
11. Plot out dose response curves for each segregated data item in the list returned from the above filter function using `ms_ITDR_ggplotting()` or `ms_ITTR_ggplotting()`.  
The arguments and its default value includes:  
...
12. Export the scaled data (virtually any data in the current working space) using `ms_filewrite()` to a local txt file.  
```{r, eval=FALSE}
...
13. Read in the exported data (virtually any data in the current working space) using `ms_fileread()` from a saved txt file.  
```{r, eval=FALSE}
...
14. Focus on a list of target proteins to study using `ms_subsetting()`.  
