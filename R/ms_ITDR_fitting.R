  ...
  for (i in seq_len(nrowdata)) {
    ...
    if (class(fit.dat) != "try-error") {
      ...
      if (mean(fitted.values(fit.dat),na.rm=T) > 1.0) {
        if (calMDT) {
          EC <- drc::ED(fit.dat, respLev=(1+nMAD*baselineMAD), interval="delta", reference="control",
                   type="absolute", lref=1.0, display=FALSE)[1,1]
          EC50 <- drc::ED(fit.dat, respLev=50, display=FALSE)[1,1]#coeffs[4,1]
        } else {
          EC <- drc::ED(fit.dat, respLev=(1+nMAD*baselineMAD)*(1+fc), interval="delta", reference="control",
                   type="absolute", lref=1.0, display=FALSE)[1,1]
          EC50 <- drc::ED(fit.dat, respLev=50, display=FALSE)[1,1]#coeffs[4,1]
        }
      } else {
        if (calMDT) {
          EC <- drc::ED(fit.dat, respLev=(1-nMAD*baselineMAD), interval="delta", reference="upper",
                   type="absolute", uref=1.0, display=FALSE)[1,1]
          EC50 <- drc::ED(fit.dat, respLev=50, reference="upper", display=FALSE)[1,1]#coeffs[4,1]
        } else {
          EC <- drc::ED(fit.dat, respLev=(1-nMAD*baselineMAD)/(1+fc), interval="delta", reference="upper",
                   type="absolute", uref=1.0, display=FALSE)[1,1]
          EC50 <- drc::ED(fit.dat, respLev=50, reference="upper", display=FALSE)[1,1]#coeffs[4,1]
        }
      }
