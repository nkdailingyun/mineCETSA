...
#' @param witherrorbar whether to plot in a mean +/- errorbar(se) graph format, default to FALSE
...
#' c("xsqrt","xcubert","xlog10","xlog2","xlinear"), corresponding to square-root, cube-root,
#' log10, log2, and linear transformation
...
ms_ITTR_ggplotting <- function(data, legenddata=NULL, levelvector=NULL, nread=10,
  ...
  numtimevector <- as.numeric(names(data)[c(3:(nread+2))])
  checkpos <- c(1,2)
  if (withset) {
    setpos <- grep("^set", names(data))
    checkpos <- c(checkpos, setpos)
  ...
  if (orderAUC) {
    if (simpleAUC==TRUE) {
      ...
      data$AUC <- apply(data, 1, function(x) auc(numtimevector, x[c(3:(nread+2))], type="linear"))
    }
  ...
  if (orderET | orderAUC | orderRep | length(plotseq)) {
  ...
  if (length(bottomlabel)==0) {
    bottomlabel <- paste0("Treatment time(", unit, ")")
  }
  ...
  if (external) { external_graphs(F) } # switch off the external graphs
  message("ITTR plot file generated successfully.")
}
