...
ms_ITTR_rawread <- function(filevector, fchoose=FALSE,
                            ...
                            nread=10, abdread=TRUE, PD21=TRUE, PD22=FALSE,
  ...
  if (flength < 2) {
    ...
    data <- ms_innerread(filevector, fchoose, treatment=time, nread, abdread, PD21, PD22, refchannel, channels)
    ...
    dirname_l <- unlist(strsplit(dirname, split="/"))
    dirname <- dirname_l[length(dirname_l)]
    indata <- ms_innerread(filevector[1], fchoose, treatment=time, nread, abdread, PD21, PD22, refchannel, channels)
    indata <- mutate(indata, condition = paste0(condition,".1"))
    outdata <- indata
    for (i in 2:flength) {
      indata <- ms_innerread(filevector[i], fchoose, treatment=time, nread, abdread, PD21, PD22, refchannel, channels)
